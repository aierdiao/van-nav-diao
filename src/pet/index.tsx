...
const STATE_META: Record<
  PetState,
  { row: number; frames: number; interval: number }
> = {
...
const DARK_POSITIONS: PositionKey[] = [
  "right-rest",
  "left-rest",
  "right-perch",
];
...
const getPositionGroup = (mode: PetMode) =>
  mode === "dark" ? DARK_POSITIONS : LIGHT_POSITIONS;
...
const pickNextPosition = (
  ...
  const filtered = avoidTagArea
    ? group.filter((item) => !item.startsWith("tag-"))
    : group;
...
const getDestinationPoint = (key: PositionKey): PetPoint => {
  ...
  const available = Math.max(width, zone?.width || window.innerWidth - 90);
  const positions: Record<PositionKey, number> = {
    "tag-left": 0.08,
    "tag-center": 0.5,
    "tag-right": 0.88,
    "left-perch": 0.05,
...
const pickWeightedLine = (
  ...
  const kind =
    forceKind || choose(mode === "dark" ? darkWeights : lightWeights, "nav");
  const candidates = DIALOG_LINES.filter(
    (line) => line.kind === kind && !recent.includes(line.text),
  );
  const fallback =
    DIALOG_LINES.find((line) => line.kind === kind) || DIALOG_LINES[0];
...
const ColaPet: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const visible = () => setActive(document.visibilityState === "visible");
  ...
  const rememberLine = (text: string) => {
    recentLinesRef.current = [
      text,
      ...recentLinesRef.current.filter((item) => item !== text),
    ].slice(0, 14);
  };
  ...
  const showDialog = (forceKind?: DialogKind) => {
    ...
    if (hideDialogTimerRef.current)
      window.clearTimeout(hideDialogTimerRef.current);
  ...
  const setPetState = useCallback((nextState: PetState) => {
    ...
    if (previousSpriteTimerRef.current)
      window.clearTimeout(previousSpriteTimerRef.current);
  ...
  const restStateForMode = useCallback(() => getRestStateForMode(mode), [mode]);

  const runBriefMotion = useCallback(
    (nextState: PetState, duration = 4200) => {
  ...
  const getLivePosition = useCallback((): PetPoint => {
    ...
    const zone = document.querySelector("#pet-zone")?.getBoundingClientRect();
    return { x: box.x - (zone?.x || 0), y: box.y - (zone?.y || 0) };
  }, []);

  const moveTo = useCallback(
    async (nextKey: PositionKey, forceDuration?: number) => {
      if (document.hidden) return;
      ...
      const duration =
        forceDuration || clamp(Math.round(distance / speed), 900, 5600);
  ...
  const moveAway = () => {
    ...
    const dialogKind: DialogKind = isSleepingState(stateRef.current)
      ? "sleepTouch"
      : "touch";
    setHandVisible(false);
    if (!reduced) void moveTo(next);
  ...
  useEffect(() => {
    ...
    const initialKey =
      modeRef.current === "dark" ? "right-rest" : "right-perch";
  ...
  useEffect(() => {
    if (meta.frames <= 1 || !active || reduced) return undefined;
    const interval = window.setInterval(() => {
      setFrame((current) => (current + 1) % meta.frames);
    }, meta.interval);
    return () => window.clearInterval(interval);
  }, [meta.frames, meta.interval, active, reduced]);
  ...
  useEffect(() => {
    if (!assetReady || !active || reduced) return undefined;
  ...
  }, [assetReady, mode, active, reduced]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [assetReady, mode, moveTo, positionKey, active, reduced]);
  ...
  }, [assetReady, mode, runBriefMotion, active, reduced]);
  ...
  return (
    <>
      {dialog && (
        <div className={`cola-dialog cola-pet--${mode}`}>
          <div
      ...
      >
        ...
        >
          ...
          {handVisible && (
            <span
              className="cola-pet__petting-hand"
              style={handStyle}
              aria-hidden="true"
            >
