...
- **健康检测访问受限状态**：HTTP 403 不再误判为失效，新增橙色「访问受限」标签（区别于绿色正常和红色失效）
- **健康检测状态筛选器**：结果表格左上方新增筛选下拉，支持按全部 / 正常 / 访问受限 / 失效过滤
- **统计区扩展为 4 列**：新增「访问受限」计数，整理失效链接不影响访问受限的条目

### 🐛 Bug Fixes

- **CardV2 useMemo 依赖缺失**：`handleImageError` 改用 `useCallback`，移除 `<img>` 外层多余 `useMemo`，消除 ESLint 警告
