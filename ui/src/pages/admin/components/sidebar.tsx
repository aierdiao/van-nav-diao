      ...
      <nav className="pt-4 relative h-full">
        {items.map((item) => (
          ...
          >
            ...
            {expanded && <span className="ml-3 truncate text-gray-700 dark:text-gray-300">{item.label}</span>}
          </Link>
        ))}
        <button
        ...
        <a
          href="https://github.com/aierdiao/van-nav-diao"
          target="_blank"
        ...
        >
          ...
          {expanded && <span className="ml-2 text-gray-600 dark:text-gray-400">GitHub</span>}
        </a>
      </nav>
    </div>
  );
