...
// 确认导入
  ...
  return (
    <div className="overflow-auto">
      ...
      {/* 部署版本信息 */}
      <Card title={t("admin.settings.version.title")} style={{ marginTop: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ fontSize: 16, color: "#666" }}>{t("admin.settings.version.current")}</span>
          <span style={{ fontSize: 20, fontWeight: 600, color: "#1890ff", fontFamily: "monospace" }}>
            {deploymentVersion}
          </span>
          <span style={{ fontSize: 13, color: "#999", marginLeft: "auto" }}>
            {t("admin.settings.version.format")}
          </span>
        </div>
      </Card>
    </div>
